...
/** Trait representing a tree of played cards. An every node of the tree has a played card.
  * If the tree is not empty, its root has a starting card played.
  * @contentDiagram
  * @author Krzysztof Piesiewicz
  */
trait TreeOfCards {
  /** @return tree of played cards with {@code subtree} attached
    * @throws Exception if the {@code subtree} cannot be attached
    *
    * @param subtree subtree of played cards to attach
    */
  def attachSubtree(subtree: CardNode): TreeOfCards
}

/** Trait describing a root node of rooted subtree of cards. It is extended by absolute root tree ([[TreeWithCards]]) and inner node ([[CardInnerNode]]).
  * @tparam N type extending [[CardNode]]
  */
trait CardNode {
  type N <: CardNode

  /** Sequnece of children nodes of this node */
  val children: Seq[CardInnerNode]

  /** Played card put in this card node */
  val playedCard: PlayedCard[_ <: Card]

  /** This card node with {@code children} replaced by {@code newChildren}
    * @param newChildren
    * @return
    */
  def withChildren(newChildren: Seq[CardInnerNode]): N

  /** Returns a tree of {@code this} with {@code subtree} attached to the corresponding place in the tree of {@code this} if there is such a place.
